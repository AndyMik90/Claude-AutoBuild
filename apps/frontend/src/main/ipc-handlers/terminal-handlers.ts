...
export function registerTerminalHandlers(
  ...
  ipcMain.handle(IPC_CHANNELS.TERMINAL_DESTROY, async (_, id: string): Promise<IPCResult> => {
    return terminalManager.destroy(id);
  });

  ipcMain.on(IPC_CHANNELS.TERMINAL_INPUT, (_, id: string, data: string) => {
    terminalManager.write(id, data);
  });

  ipcMain.on(IPC_CHANNELS.TERMINAL_RESIZE, (_, id: string, cols: number, rows: number) => {
    terminalManager.resize(id, cols, rows);
  });

  ipcMain.on(IPC_CHANNELS.TERMINAL_INVOKE_CLAUDE, (_, id: string, cwd?: string) => {
  ...
  ipcMain.on(IPC_CHANNELS.TERMINAL_SET_TITLE, (_, id: string, title: string) => {
  ...
  ipcMain.handle(
    ...
    async (_, profileId: string): Promise<IPCResult> => {
      debugLog(
        "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] ========== PROFILE SWITCH START =========="
      );
      ...
      try {
        ...
        debugLog(
          "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] New profile:",
          newProfile
            ? {
                id: newProfile.id,
        ...
        if (profileChanged) {
          ...
          debugLog(
            "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] Active terminal IDs:",
            activeTerminalIds
          );
          ...
          for (const terminalId of activeTerminalIds) {
            ...
            if (isClaudeMode) {
              ...
              debugLog(
                "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] Queuing terminal for profile switch:",
                terminalId
              );
              switchPromises.push(
                terminalManager
                  .switchClaudeProfile(terminalId, profileId)
                  .then(() => {
                    debugLog(
                      "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] Terminal profile switch SUCCESS:",
                      terminalId
                    );
                  })
                  .catch((err) => {
                    debugError(
                      "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] Terminal profile switch FAILED:",
                      terminalId,
                      err
                    );
          ...
          if (switchPromises.length > 0) {
            debugLog(
              "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] Waiting for",
              switchPromises.length,
              "terminal switches..."
            );
            ...
            debugLog(
              "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] No terminals in Claude mode to switch"
            );
          }
        } else {
          debugLog(
            "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] Same profile selected, no terminal switches needed"
          );
        }

        debugLog(
          "[terminal-handlers:CLAUDE_PROFILE_SET_ACTIVE] ========== PROFILE SWITCH COMPLETE =========="
        );
        return { success: true };
      } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (_, profileId: string): Promise<IPCResult> => {
      try {
        ...
        // Get the shell type from the terminal for appropriate command syntax
        const terminal = terminalManager.getTerminal(terminalId);
        const shellType = terminal?.shellType || "unknown";
        debugLog("[IPC] Terminal shell type:", shellType);

        // Build the login command with the profile's config dir
        // Use shell-type-specific syntax and escaping for environment variables
        ...
        const pathPrefix = claudeEnv.PATH
          ? buildPathAssignment(claudeEnv.PATH, shellType) + " && "
          : "";

        if (!profile.isDefault && profile.configDir) {
          // Build environment variable assignment and echo command for the shell type
          const envVarAssignment = buildEnvVarAssignment(
            "CLAUDE_CONFIG_DIR",
            profile.configDir,
            shellType
          );
          const envVarReference = buildEnvVarReference("CLAUDE_CONFIG_DIR", shellType);
          const claudeInvocation = buildCommandInvocation(claudeCmd, ["setup-token"], shellType);

          // Build echo command for verification
          let echoCommand: string;
          if (shellType === "powershell") {
            echoCommand = `Write-Host "Config dir: ${envVarReference}"`;
          ...
          // Default profile - no config dir needed
          const claudeInvocation = buildCommandInvocation(claudeCmd, ["setup-token"], shellType);
          loginCommand = `${pathPrefix}${claudeInvocation}`;
        }
  ...
  ipcMain.handle(
    ...
    async (
      _,
      settings: Partial<import("../../shared/types").ClaudeAutoSwitchSettings>
    ): Promise<IPCResult> => {
      try {
        const profileManager = getClaudeProfileManager();
  ...
  ipcMain.on(IPC_CHANNELS.TERMINAL_RESUME_CLAUDE, (_, id: string, sessionId?: string) => {
    // Use async version to avoid blocking main process during CLI detection
  ...
  ipcMain.on(IPC_CHANNELS.TERMINAL_ACTIVATE_DEFERRED_RESUME, (_, id: string) => {
  ...
  ipcMain.handle(IPC_CHANNELS.TERMINAL_GET_SESSION_DATES, async (_, projectPath?: string) => {
    try {

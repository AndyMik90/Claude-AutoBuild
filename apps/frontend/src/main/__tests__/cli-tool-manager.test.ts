...
vi.mock("../env-utils", () => ({
  ...
  prepareShellCommand: vi.fn((command: string) => {
    // Mock prepareShellCommand to match actual behavior
    const needsShell = process.platform === "win32" && /\.(cmd|bat)$/i.test(command);
    return { command, needsShell };
  }),
  preparePythonSubprocessCommand: vi.fn((command: string) => {
    // Mock preparePythonSubprocessCommand to match actual behavior
    const needsShell = process.platform === "win32" && /\.(cmd|bat)$/i.test(command);
    return { command, needsShell };
  }),
...
describe("cli-tool-manager - Claude CLI NVM detection", () => {
  ...
  describe("NVM path detection on Unix/Linux/macOS", () => {
    ...
    it("should validate Claude CLI before returning NVM path", () => {
      ...
      const mockDirents = [{ name: "v22.17.0", isDirectory: () => true }];
      vi.mocked(readdirSync).mockReturnValue(mockDirents as any);
...
describe("cli-tool-manager - Helper Functions", () => {
  describe("getClaudeDetectionPaths", () => {
    ...
    it("should return Unix paths on Linux", () => {
      ...
      expect(
        paths.platformPaths.some(
          (p) => p.includes(".local") && p.includes("bin") && p.includes("claude")
        )
      ).toBe(true);
...
describe("cli-tool-manager - Claude CLI Windows where.exe detection", () => {
  ...
  it("should validate Claude CLI before returning where.exe path", () => {
    ...
    vi.mocked(findWindowsExecutableViaWhere).mockReturnValue("D:\\Tools\\claude.cmd");

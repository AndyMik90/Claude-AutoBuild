      ...
      <CardContent className="space-y-3">
        ...
        <div className="grid gap-2 text-xs">
          ...
          {service.testing && (
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span>{t('projectIndex.service.testing')}: {service.testing}</span>
            </div>
          )}
          {service.orm && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Database className="h-3 w-3 shrink-0" />
              <span>{t('projectIndex.service.orm')}: {service.orm}</span>
            </div>
          )}
          {service.default_port && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Globe className="h-3 w-3 shrink-0" />
              <span>{t('projectIndex.service.port')}: {service.default_port}</span>
            </div>
          )}
          {service.styling && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Code className="h-3 w-3 shrink-0" />
              <span>{t('projectIndex.service.styling')}: {service.styling}</span>
            </div>
          )}
          {service.state_management && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Package className="h-3 w-3 shrink-0" />
              <span>{t('projectIndex.service.state')}: {service.state_management}</span>
            </div>
          )}
        </div>
        ...
        {service.apple_frameworks && service.apple_frameworks.length > 0 && (
          <div className="pt-2 border-t border-border">
            <p className="text-xs text-muted-foreground mb-1.5">{t('projectIndex.service.appleFrameworks')}</p>
        ...
        {service.spm_dependencies && service.spm_dependencies.length > 0 && (
          <div className="pt-2 border-t border-border">
            <p className="text-xs text-muted-foreground mb-1.5">{t('projectIndex.service.spmDependencies')}</p>
            <div className="flex flex-wrap gap-1">
        ...
        {service.key_directories && Object.keys(service.key_directories).length > 0 && (
          <div className="pt-2 border-t border-border">
            <p className="text-xs text-muted-foreground mb-1.5">{t('projectIndex.service.keyDirectories')}</p>
            <div className="flex flex-wrap gap-1">

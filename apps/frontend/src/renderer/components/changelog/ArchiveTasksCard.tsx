  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setArchiveError(result.error || t('changelog.archive.failedToArchive'));
      }
    } catch (err) {
      setArchiveError(err instanceof Error ? err.message : t('changelog.archive.failedToArchive'));
    } finally {
        ...
        {archiveSuccess ? (
          <div className="flex items-center gap-2 text-success">
            ...
            <span className="text-sm">
              {t('changelog.archive.archivedMessage', { count })}
            </span>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              {t('changelog.archive.description', { count })} {t('changelog.archive.descriptionSuffix')}
            </p>
            {archiveError && (
            ...
            >
              {isArchiving ? (
                ...
                <>
                  ...
                  {t('changelog.archive.archiveTasks', { count })}
                </>
              )}
            </Button>
          </div>

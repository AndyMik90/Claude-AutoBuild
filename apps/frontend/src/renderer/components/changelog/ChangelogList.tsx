...
export function ChangelogList({
  ...
  const getSummaryLabel = () => {
    switch (sourceMode) {
      case 'tasks':
        return t('changelog.list.task');
      case 'git-history':
      case 'branch-diff':
        return t('changelog.list.commit');
      default:
        return t('changelog.list.item');
    }
  };
            ...
            {doneTasks.length === 0 ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('changelog.list.noCompletedTasks')}</h3>
      ...
      {(sourceMode === 'git-history' || sourceMode === 'branch-diff') && (
        <>
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('changelog.list.commitsFound', { count: previewCommits.length })} {t('changelog.list.commitsFound_plural', { count: previewCommits.length })}
              </span>
              {isLoadingCommits && (
            ...
            {isLoadingCommits ? (
              ...
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('changelog.list.noCommitsFound')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('changelog.list.noCommitsFoundGitHistory')
                      : t('changelog.list.noCommitsFoundBranchDiff')}
                  </p>
                </div>
              </div>
            ) : (

    ...
    <div className="w-80 shrink-0 border-r border-border overflow-y-auto">
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-3">
          <Label className="text-sm font-medium">{t('changelog.changelogSource')}</Label>
          <RadioGroup
            value={sourceMode}
        ...
        {sourceMode === 'git-history' && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">{t('changelog.historyOptions.title')}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* History Type */}
              <div className="space-y-2">
                <Label className="text-xs">{t('changelog.historyOptions.historyType')}</Label>
                <Select
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="since-date">
                      <div className="flex items-center gap-2">
                        ...
                        {t('changelog.historyOptions.sinceDate')}
                      </div>
                    </SelectItem>
              ...
              {gitHistoryType === 'since-date' && (
                <div className="space-y-2">
                  <Label className="text-xs">{t('changelog.historyOptions.sinceDate')}</Label>
                  <Input
                    type="date"
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <Select value={gitHistoryFromTag} onValueChange={onGitHistoryFromTagChange}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('changelog.historyOptions.selectTag')} />
                      </SelectTrigger>
                      <SelectContent>
                        {tags.map((tag) => (
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={gitHistoryToTag || 'HEAD'} onValueChange={(v) => onGitHistoryToTagChange(v === 'HEAD' ? '' : v)}>
                      <SelectTrigger>
                        <SelectValue placeholder={t('changelog.historyOptions.headLatest')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="HEAD">{t('changelog.historyOptions.headLatest')}</SelectItem>
                        {tags.map((tag) => (
                          <SelectItem key={tag.name} value={tag.name}>
                            {tag.name}
                          </SelectItem>
              ...
              {gitHistoryType === 'since-version' && (
                <div className="space-y-2">
                  ...
                  <Select value={gitHistorySinceVersion} onValueChange={onGitHistorySinceVersionChange}>
                    <SelectTrigger>
                      <SelectValue placeholder={t('changelog.historyOptions.selectVersion')} />
                    </SelectTrigger>
                    <SelectContent>
                      {tags.map((tag) => (
                        <SelectItem key={tag.name} value={tag.name}>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={baseBranch} onValueChange={onBaseBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('changelog.selectBaseBranch')} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select value={compareBranch} onValueChange={onCompareBranchChange}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('changelog.selectCompareBranch')} />
                  </SelectTrigger>
                  <SelectContent>
                    {localBranches.map((branch) => (
                      <SelectItem key={branch.name} value={branch.name}>

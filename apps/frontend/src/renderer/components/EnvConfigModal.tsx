...
export function EnvConfigModal({
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (tokenResult.success && tokenResult.data) {
          ...
          setError(tokenResult.error || t('envConfig.errors.failedToCheckTokenStatus'));
        }
  ...
  const handleUseExistingProfile = async () => {
    ...
    try {
      ...
      if (!profile?.oauthToken) {
        setError(t('envConfig.errors.invalidProfile'));
        setIsSaving(false);
        return;
      }
      ...
      if (result.success) {
        ...
        setError(result.error || t('envConfig.errors.failedToSaveToken'));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t('envConfig.errors.unknownError'));
    } finally {
  ...
  const handleAuthenticateWithBrowser = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || t('envConfig.errors.failedToStartAuth'));
        setIsAuthenticating(false);
      }
      // Keep isAuthenticating true - will be cleared when token is received
    } catch (err) {
      setError(err instanceof Error ? err.message : t('envConfig.errors.failedToStartAuth'));
      setIsAuthenticating(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!token.trim()) {
      setError(t('envConfig.errors.pleaseEnterToken'));
      return;
    }
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t('envConfig.authenticatedAccounts', {
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {claudeProfiles.length > 0
                          ? t('envConfig.addNewAccountDesc')
                          : t('envConfig.clickBelowDesc')
                        }
                      </p>
                    </div>
                  </div>
            ...
            {hasExistingToken && (
              <div className="rounded-lg bg-muted/50 p-3">
                <p className="text-sm text-muted-foreground">
                  {t('envConfig.existingTokenInfo')} {showManualEntry ? t('envConfig.existingTokenReplace') : t('envConfig.existingTokenAuthReplace')}
                </p>
              </div>
            )}
          </div>
  ...
  const checkToken = useCallback(async () => {
    ...
    try {
      ...
      if (!result.success && !hasAPIProfile) {
        setError(result.error || t('envConfig.errors.failedToCheckToken'));
      }
    } catch (err) {

...
// Forgejo issue state colors
const FORGEJO_ISSUE_STATE_COLORS: Record<string, string> = {
  open: 'bg-green-500/10 text-green-500 border-green-500/20',
  closed: 'bg-purple-500/10 text-purple-500 border-purple-500/20'
};

const FORGEJO_COMPLEXITY_COLORS: Record<string, string> = {
...
export function IssueDetail({ issue, onInvestigate, investigationResult, linkedTaskId, onViewTask }: IssueDetailProps) {
  const { t } = useTranslation(['forgejo', 'common']);
        ...
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className={`${FORGEJO_ISSUE_STATE_COLORS[issue.state] || ''}`}
              >
                {t(`states.${issue.state}`)}
              </Badge>
              <span className="text-sm text-muted-foreground">#{issue.number}</span>
            </div>
            <Button variant="ghost" size="icon" asChild aria-label="Open on Forgejo">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
          ...
          {issue.commentsCount > 0 && (
            <div className="flex items-center gap-1">
              ...
              {issue.commentsCount} {t('detail.comments')}
            </div>
          )}
        </div>
            ...
            <CardContent className="text-sm space-y-2">
              {investigationResult?.success ? (
                <>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge className={FORGEJO_COMPLEXITY_COLORS[investigationResult.analysis.estimatedComplexity]}>

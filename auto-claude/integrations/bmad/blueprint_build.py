...
class BlueprintBuildOrchestrator:
    ...
    def __init__(
        ...
        if self.bmad_loader.is_available():
            print(
                f"[BMAD] Agents available: {', '.join(self.bmad_loader.list_agents())}"
            )
        else:
        ...
        if self.workflow_loader.is_available():
            print(
                f"[BMAD] Workflow phases: {', '.join(self.workflow_loader.list_workflow_phases())}"
            )
        else:
    ...
    async def run(self) -> bool:
        ...
        while True:
            ...
            self.manager.update_status(
                component.id, ComponentStatus.IN_PROGRESS, "Started build"
            )
            ...
            try:
                ...
                if not build_success:
                    self.manager.update_status(
                        component.id, ComponentStatus.FAILED, "Build failed"
                    )
                ...
                self.manager.update_status(
                    component.id, ComponentStatus.VERIFYING, "Running verification"
                )
                ...
                if verification_result.passed:
                    ...
                    for i, criterion in enumerate(component.acceptance_criteria):
                        self.manager.mark_criterion_verified(
                            component.id, i, f"Verified during build #{iterations}"
                        )
                else:
                    self.manager.update_status(
                        component.id,
    ...
    async def _build_component(self, component) -> bool:
        ...
        try:
            ...
            if self.bmad_loader.is_available():
                ...
                if self.workflow_loader.is_available():
                    workflow_context = self.workflow_loader.get_phase_context(
                        "implementation"
                    )
                    if workflow_context:
    ...
    def _create_build_prompt(self, component) -> str:
        ...
        criteria_text = "\n".join(
            f"  - {ac.description}" for ac in component.acceptance_criteria
        )

        files_text = (
            "\n".join(f"  - {f}" for f in component.files)
            if component.files

...
class BMADWorkflowLoader:
    ...
    def _get_workflows_path(self) -> Optional[Path]:
                ...
                if workflow_dir.is_dir():
                    ...
                    if workflow_file.exists():
                        ...
                        if workflow_data:
                            workflows.append({
                                "phase": phase_name,
                                "name": workflow_data.get("name", workflow_dir.name),
    ...
    def _load_workflow(self, workflow_path: Path) -> Optional[dict]:
    ...
    def get_workflow_instructions(self, workflow_path: str) -> str:
        ...
        if installed_path:
            ...
            installed_path = installed_path.replace("{project-root}", str(self.bmad_path))
        ...
        if instructions_ref:
            instructions_ref = instructions_ref.replace("{project-root}", str(self.bmad_path))
            instructions_ref = instructions_ref.replace("{installed_path}", installed_path)

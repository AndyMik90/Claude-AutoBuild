...
class StepFileLoader:
    ...
    def resolve_path(self, path_ref: str, workflow_path: Optional[Path] = None) -> Path:
        """
    ...
    def load_step(
        ...
        if self.token_budget:
            if not self.token_budget.can_afford(TokenCategory.STEP_CONTENT, token_count):
    ...
    def _parse_xml_step(
        ...
        for match in step_matches:
            if int(match[0]) == step_number:
                ...
                actions = [
                    a.strip()
                    for a in self.ACTION_PATTERN.findall(step_content)
                ]

                # Extract checks
                for check_match in self.CHECK_PATTERN.findall(step_content):
                    checks.append({
                        'condition': check_match[0],
                        'content': check_match[1].strip()
                    })

                # Extract outputs
                outputs = [
                    o.strip()
                    for o in self.OUTPUT_PATTERN.findall(step_content)
                ]

                # Extract critical notes
                critical_notes = [
                    c.strip()
                    for c in self.CRITICAL_PATTERN.findall(step_content)
                ]

...
class WorkflowParser:
    ...
    def parse(self, file_path: Path, force: bool = False) -> Optional[ParsedWorkflow]:
        """
        ...
        if self.token_budget:
            if not self.token_budget.can_afford(TokenCategory.WORKFLOW_DEF, token_count):
                return None
    ...
    def _parse_markdown(self, content: str, file_path: Path) -> ParsedWorkflow:
        ...
        for handler_match in handlers:
            workflow.checkpoint_handlers.append(CheckpointHandler(
                key=handler_match[0],
                action=handler_match[1].strip()
            ))
    ...
    def _infer_module_phase(self, workflow: ParsedWorkflow, file_path: Path) -> None:
        ...
        phases = ['1-analysis', '2-plan-workflows', '3-solutioning', '4-implementation',
                  'bmad-quick-flow', 'bmgd-quick-flow', 'testarch', 'gametest']
        for part in path_parts:
            if part.lower() in phases or part.lower().startswith(('1-', '2-', '3-', '4-')):
                workflow.phase = part.lower()
                break

    def list_workflows(self, module: Optional[str] = None) -> List[Path]:
        """
        ...
        if module:
            if module == 'core':
                ...
                search_paths = [self.bmad_root / 'src' / 'modules' / module / 'workflows']
        else:
            search_paths = [
    ...
    def get_workflow_by_name(self, name: str) -> Optional[ParsedWorkflow]:
        """

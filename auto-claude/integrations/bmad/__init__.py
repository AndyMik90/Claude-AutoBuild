...
# Shared services
from .shared.token_budget import TokenBudget, TokenCategory, TokenAllocation
from .shared.cache import DiskLRUCache, CacheEntry
from .shared.step_loader import StepFileLoader, StepContent, StepReference
from .shared.workflow_parser import WorkflowParser, ParsedWorkflow, WorkflowFormat

# Module loaders
from .modules.core_loader import CoreModuleLoader, AgentPersona as CoreAgent
from .modules.bmm_loader import BMMModuleLoader, BMMAgent, BMMAgentRole, BMMPhase
from .modules.bmgd_loader import BMGDModuleLoader, BMGDAgent, BMGDAgentRole, BMGDPhase
from .modules.cis_loader import CISModuleLoader, CISAgent, CISAgentRole, CISWorkflowType
from .modules.bmb_loader import BMBModuleLoader, BMBAgent
...
class BMADIntegration:
    ...
    def __init__(
        self,
        bmad_path: Optional[Path] = None,
        cache_dir: Optional[Path] = None,
        token_budget: int = 50_000,
        enabled_modules: Optional[List[ModuleType]] = None
    ):
        """
        ...
        """
        # Resolve paths
        self.bmad_path = Path(bmad_path or os.environ.get('BMAD_PATH', self.DEFAULT_BMAD_PATH))
        self.cache_dir = Path(cache_dir or os.environ.get('BMAD_CACHE', self.DEFAULT_CACHE_DIR))
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'modules_loaded': {
                m.value: loader.is_available() if hasattr(loader, 'is_available') else False
    ...
    def list_agents(self, module: Optional[ModuleType] = None) -> List[AgentInfo]:
        """
    ...
    def list_workflows(self, module: Optional[ModuleType] = None) -> List[WorkflowInfo]:
        """
        ...
        for mod_type, loader in self._loaders.items():
            ...
            for wf_name in loader.list_workflows() if hasattr(loader, 'list_workflows') else []:
                wf = loader.get_workflow(wf_name) if hasattr(loader, 'get_workflow') else None
                if wf:
                    workflows.append(WorkflowInfo(
                        id=wf_name,
                        name=wf.name if hasattr(wf, 'name') else wf_name,
                        module=mod_type,
                        phase=wf.phase.value if hasattr(wf, 'phase') and hasattr(wf.phase, 'value') else str(getattr(wf, 'phase', '')),
    ...
    def get_workflow(self, workflow_id: str) -> Optional[ParsedWorkflow]:
        """

...
class VerificationGate:
    ...
    def _check_files_exist(self, component) -> VerificationResult:
        ...
        if not component.files:
            return VerificationResult(
                passed=True,
                message="No specific files required"
            )
        ...
        return VerificationResult(
            passed=True,
            message=f"All {len(component.files)} files exist"
        )
    ...
    async def _run_component_tests(self, component) -> VerificationResult:
        ...
        if not test_files:
            return VerificationResult(
                passed=True,
                message="No component-specific tests found (skipped)"
            )
        ...
        try:
            ...
            return VerificationResult(
                passed=True,
                message="pytest not available (skipped)"
            )
    ...
    async def _check_acceptance_criteria(self, component) -> VerificationResult:
        ...
        if passed_count == total_count:
            return VerificationResult(
                passed=True,
                message=f"All {total_count} acceptance criteria validated"
            )
        else:
    ...
    async def verify_single_criterion(
        ...
        if criterion_index >= len(component.acceptance_criteria):
            return VerificationResult(
                passed=False,
                message="Invalid criterion index"
            )
        ...
        return VerificationResult(
            passed=True,
            message=f"Criterion verified: {criterion.description}"
        )
